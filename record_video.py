...
if __name__ == '__main__':
    ...
    try:
        ...
        name = "video"
        while done is False:
            query = input('Type q to quit, n to change name, t for changing time, Enter to continue: ').strip()
            if query is 'q':
                done = True
            elif query is 'n':
                name = input("Type name: ")
            elif query is 't':
                VIDEO_LENGTH = int(input("Time in seconds: "))
            else:
                print(f'Waiting {WAIT} seconds...')
                time.sleep(WAIT)
                play_sound()
                while True:
                    path = os.path.join(output_dir, '{}_{}.avi'.format(name, counter))
                ...
                fname = f"videos/{name}_depth_{counter}.json"
